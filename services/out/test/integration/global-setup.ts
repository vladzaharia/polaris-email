// Node-side global setup for the out-workers pool. We read the D1 migration
...
export default async function setup(project: TestProject): Promise<void> {
  const migrationsDir = path.resolve(here, '../../../api/migrations');
  ...
  // fails with "no such table: main.messages_old". The services/out worker
  // never queries these views or triggers, so stripping them keeps the test
  // honest without diverging from production schema.
