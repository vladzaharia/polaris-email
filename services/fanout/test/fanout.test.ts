...
function mkBatch(body: unknown) {
  ...
  } as unknown as MessageBatch<typeof body>;
}
...
describe('fanout v2 envelope + delivered transition', () => {
  it('emits v2 envelope with X-Polaris-Sig: v2=… and message JSON', async () => {
    const db = new FakeDB();
    db.messages.push({
      id: 'M1',
      mailbox_id: 'MB1',
    ...
    const env = { DB: db as unknown as D1Database, R2: r2 as unknown as R2Bucket };
    await worker.queue(
      mkBatch({
        event_id: 'EV1',
    ...
    expect(captured!.headers['x-polaris-sig']?.startsWith('v2=')).toBe(true);
