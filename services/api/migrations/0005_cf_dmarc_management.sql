...
--
-- Cloudflare DMARC Management takes over the DMARC report ingest. This
-- migration drops the legacy ARF-inbox stack:
--
--   * dmarc_aggregate_reports — full per-report storage (CF holds these)
--   * mail_domains.dmarc_rua — operator RUA tag (CF owns the record)
--   * mail_domains.dmarc_record_managed_by_polaris — CF always manages
--   * the platform-DMARC mailbox row + its principal/sender/receiver rows
--   * the dmarc.claim_management and dmarc_aggregate_report.ingest audit
--     actions (no longer reachable)
--
...
-- ============================================================================
-- audit_log CHECK rebuild — drop the two legacy actions from the allow list.
-- SQLite has no ALTER TABLE ... ALTER CONSTRAINT, so copy → drop → rename.
-- The chained-hash invariant survives because rows are copied verbatim
-- (prev_hash + row_hash carry over).
-- ============================================================================
...
VALUES (5, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), '0005_cf_dmarc_management');
