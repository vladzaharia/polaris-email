-- polaris-email — extend audit_log CHECK with webhook_sub.rotate
-- (schema version=22)
--
-- SQLite cannot ALTER a CHECK constraint in place. Rebuild audit_log using
-- the standard rename/copy/drop pattern. The hash chain is preserved
-- byte-for-byte because we copy id/prev_hash/row_hash verbatim.
...
VALUES (22, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), '0022_audit_action_webhook_rotate');
