-- 0009_bridges_cf_dns_token.sql
--
-- Per-bridge Cloudflare API token for DNS-01 / A-record management.
-- The bridge mints its own LE cert via embedded Lego; the token id lets
-- us revoke on rotate / hard-delete. Plaintext only lives in
-- KV_KEY_CACHE under bridge_cf_dns_plain:<id> — same recovery pattern
-- as the HMAC plaintext cache (services/api/src/bridge-auth.ts:18-22).
--
-- Audit gains two new actions for the token lifecycle. SQLite has no
-- ALTER CHECK; copy → drop → rename, same pattern as 0008.

ALTER TABLE bridges ADD COLUMN cf_dns_token_id TEXT;
