-- polaris-email — extend audit_log CHECK with bootstrap.webauthn_enrolled
-- (schema version=23)
--
-- PR 7's genesis-seal flow records the WebAuthn enrolment completion as
-- an audit row, so the operator-side ceremony is in the same hash chain
-- as the original bootstrap.consume row. SQLite cannot ALTER a CHECK
-- constraint in place; rebuild audit_log using the rename/copy/drop
-- pattern. The hash chain is preserved byte-for-byte.
...
VALUES (23, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), '0023_audit_action_webauthn_enrolled');
