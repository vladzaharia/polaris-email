...
senders.post(
  ...
  async (c) => {
    ...
    await audit(c.env, {
      ...
      target: id,
      meta: { mailbox_id: mailboxId, domain_id: domainId, local_part: body.local_part, default: isDefault === 1 },
    });
    return c.json(
      {
        id,
...
senders.post(
  ...
  async (c) => {
    ...
    await audit(c.env, {
      ...
      target: id,
      meta: { sender_id: senderId, principal_id: principalId, mailbox_id: senderRow.mailbox_id, username },
    });
    return c.json(
      {
        id,
