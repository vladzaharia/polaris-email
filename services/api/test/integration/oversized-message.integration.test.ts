...
describe('A.6: oversized messages rejected pre-enqueue', () => {
  ...
  it('rfc822 path: body > 25 MiB returns 413 message_too_large', async () => {
    ...
    // Register a bridge row and capture its per-bridge HMAC key (Phase 2h
    // moved bridge auth from a global BRIDGE_HMAC_KEY env to per-bridge
    // secrets returned once on registration).
    ...
    const bridgeHmacKey = bridge.hmac_key;
