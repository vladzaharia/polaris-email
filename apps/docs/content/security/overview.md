...
description: Threat model, the HMAC signing spec, R2 unguessability boundary, audit-anchor chain (Backblaze B2 + Object Lock), credential revocation, and compliance posture.
...
- **Audit anchors live off Cloudflare.** Hourly signed anchors push to
  Backblaze B2 with Object Lock COMPLIANCE (~7-year retention). See the
  ["Audit anchors" section](/security/threat-model#audit-anchors) of
  the threat model and the
