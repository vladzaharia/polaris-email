...
`polaris-email` database. Use it when:
...
wrangler d1 time-travel info polaris-email --timestamp 2026-04-12T15:00:00Z
```
...
```sh
wrangler d1 create polaris-email-pitr

# This restores polaris-email's state at <timestamp> into the new DB.
wrangler d1 time-travel restore polaris-email-pitr \
...
1. `wrangler d1 time-travel info polaris-email --timestamp <when-was-it-fine>`
2. Restore into `polaris-email-pitr` (see above).
3. `wrangler d1 execute polaris-email-pitr --remote --command "SELECT * FROM api_keys WHERE revoked_at IS NULL"`
4. Build INSERT / UPDATE statements to apply the missing rows back into
   ...
   activity).
5. Apply the targeted statements with `wrangler d1 execute polaris-email --remote --command "..."`.
...
- `services/api/migrations/` — migration source; rollback via
  `wrangler d1 migrations rollback polaris-email --remote`.
