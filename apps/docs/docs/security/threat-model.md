...
alongside the audit-verify tooling.

:::warning Out of date
Step 2 above still references the retired `withApproval('anchor.rotate')`
two-person flow. Real deployments are single-operator; the panel gates
this destructive action client-side via `DestructiveActionDialog`
(type-the-resource-name confirmation). The `audit_log` chained-hash table
(anchored hourly to Backblaze B2) remains the canonical record of who
rotated the key. The rest of the rotation procedure (overlap window,
promote, log fingerprint) still applies as written.
:::
...
the audit-verify scheduled job under `services/api/src/scheduled/audit-verify.ts`.
...
- [ ] Audit hash-chain verified end-to-end; latest
