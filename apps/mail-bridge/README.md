...
On-prem unified mail bridge for Polaris. One Go binary, three protocols:

- **SMTPS** on `:465` — outbound submission (formerly `apps/submission-daemon`)
- **IMAP4rev2** on `:993` — INBOX retrieval, IDLE, CONDSTORE (RFC 9051 subset)
- **JMAP** on `:443` — Mailbox/Email/Identity/EmailSubmission/PushSubscription
  (RFC 8620 + 8621 + 8887 subset)

All three listeners share a single auth lookup against `mailbox_credentials`
...
A Tailscale sidecar fronts all three listeners on the same MagicDNS hostname
...
The bridge binds 465 / 993 / 443 directly to the host network. The operator
...
- **Thunderbird** — supports both IMAP (Mode 1 + 2) and JMAP (`Account
Settings → Server → JMAP`).
- **Apple Mail** — IMAP only. Configure SMTP submission with the SMTPS
  password and IMAP retrieval with the IMAP password (same `mailbox_credentials`
  row when both use auth_type='password' under different protocols, distinct
  rows otherwise).
- **fastmail-cli / aerc** — JMAP-capable.

See `docs/mail-bridge.md` for protocol-specific quirks (Apple Mail's lack of
JMAP, Thunderbird's JMAP support in v102+).
